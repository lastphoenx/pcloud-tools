...
_dns_cache: Dict[tuple, str] = {}

def _resolve_cached(host: str, port: int) -> str:
    """DNS einmal auflösen und cachen – verhindert tausende DNS-Queries bei vielen API-Aufrufen."""
    key = (host, port)
    if key not in _dns_cache:
        infos = socket.getaddrinfo(host, port, socket.AF_INET, socket.SOCK_STREAM)
        _dns_cache[key] = infos[0][4][0]
    return _dns_cache[key]

def _connect(host: str, port: int, timeout: int) -> ssl.SSLSocket:
    ip = _resolve_cached(host, port)
    raw = socket.create_connection((ip, port), timeout=timeout)
    ctx = ssl.create_default_context()
    tls = ctx.wrap_socket(raw, server_hostname=host)  # SNI braucht Hostname
...
def upload_file(cfg: dict, *, local_path: str, remote_path: str) -> dict:
    ...
    - Nutzt Keep-Alive Session für bessere Performance und Stabilität.
    - Große Dateien (≥100MB): Automatisch Chunked Upload für Robustheit.
    ...
    chunk_threshold = int(os.environ.get("PCLOUD_CHUNK_THRESHOLD", str(100 * 1024**2)))  # 100MB default
    
    # Für große Dateien: Chunked Upload (robuster bei Netzwerkfehlern)
    if file_size >= chunk_threshold:
        return _upload_file_chunked(cfg, local_path=local_path, remote_path=remote_path)
    
    # Kleine Dateien: Standard Upload (schnell, einfach)
    ...
    with open(local_path, "rb") as fh:
        files = {"file": (fname, fh)}
        # connect 60s, read 600s (10min) für Dateien <100MB
        upload_timeout = (60, 600)
        
        # WICHTIG: Nutze Keep-Alive Session statt direktem requests.post()
        session = _get_session()
        r = session.post(url, params=params, files=files, timeout=upload_timeout)
...
def _upload_file_chunked(cfg: dict, *, local_path: str, remote_path: str) -> dict:
    """
    Chunked Upload für große Dateien (robuster bei Netzwerkfehlern).
    
    3-Schritt-API:
    1. upload_create → uploadid
    2. upload_write (mehrfach) → Chunks hochladen
    3. upload_save → metadata zurück
    """
    import os
    global _session  # Für Session-Refresh bei Connection-Fehlern
    
    # 1. Zielordner sicherstellen und folderid holen (WICHTIG gegen Dateinamen-Kollisionen!)
    dest_dir = os.path.dirname(remote_path.rstrip("/")) or "/"
    fname = os.path.basename(remote_path.rstrip("/"))
    dest_folderid = ensure_path(cfg, dest_dir)
    dest_path = _norm_remote_path(dest_dir)  # Für upload_save (path + filename)
    
    file_size = os.path.getsize(local_path)
    chunk_size = int(os.environ.get("PCLOUD_CHUNK_SIZE", str(5 * 1024**2)))  # 5MB default
    
    session = _get_session()
    base_url = _rest_base(cfg)
    
    # Schritt 1: Upload-Session erstellen (nur access_token!)
    create_params = {
        "access_token": cfg["token"],
    }
    r = session.post(f"{base_url}/upload_create", params=create_params, timeout=(60, 30))
    try:
        j = r.json()
    except Exception:
        raise RuntimeError(f"upload_create: HTTP {r.status_code}, kein JSON")
    
    if j.get("result") != 0:
        raise RuntimeError(f"upload_create failed: {j}")
    
    uploadid = j.get("uploadid")
    if not uploadid:
        raise RuntimeError(f"upload_create: keine uploadid in Response: {j}")
    
    # Schritt 2: Chunks hochladen (mit Retry pro Chunk!)
    upload_offset = 0
    max_chunk_retries = int(os.environ.get("PCLOUD_CHUNK_RETRIES", "8"))
    ...
    with open(local_path, "rb") as fh:
        while upload_offset < file_size:
            ...
            # WICHTIG: Retry-Logik PRO CHUNK (nicht pro Datei!)
            chunk_uploaded = False
            last_error = None
            
            for attempt in range(1, max_chunk_retries + 1):
                try:
                    # Headers für Binary-Upload (pCloud braucht Content-Type!)
                    headers = {
                        "Content-Type": "application/octet-stream",
                        "Connection": "keep-alive"
                    }
                    # Timeout pro Chunk: connect 60s, read 120s
                    r = session.post(f"{base_url}/upload_write", params=write_params, 
                                   data=chunk_data, headers=headers, timeout=(60, 120))
                    try:
                        j = r.json()
                    except Exception:
                        raise RuntimeError(f"upload_write: HTTP {r.status_code}, kein JSON")
                    
                    if j.get("result") != 0:
                        raise RuntimeError(f"upload_write failed: {j}")
                    ...
                    if attempt < max_chunk_retries:
                        # Bei Connection-Problemen: Session erneuern (uploadid bleibt gültig!)
                        if any(err in str(e).lower() for err in ["connection", "remote", "timeout", "reset", "broken pipe"]):
                            try:
                                session.close()
                            except:
                                pass
                            ...
                            _session = None
                            session = _get_session()
                            print(f"[chunk-retry] Session erneuert wegen: {type(e).__name__}", flush=True)
                        
                        # Exponential Backoff (max 60s)
                        sleep_time = min(60.0, 1.0 * (2 ** (attempt - 1)))
                        print(f"[chunk-retry] Offset {upload_offset} fehlgeschlagen "
                              f"(Versuch {attempt}/{max_chunk_retries}, {type(e).__name__}: {e}), "
                              f"warte {sleep_time:.1f}s ...", flush=True)
                        import time
                        time.sleep(sleep_time)
                        # Sicherstellen, dass wir denselben Chunk nochmal lesen
                        fh.seek(upload_offset)
                        chunk_data = fh.read(chunk_size)
                    else:
                        raise RuntimeError(
                            f"Chunk bei Offset {upload_offset} nach {max_chunk_retries} "
                            f"Versuchen fehlgeschlagen: {last_error}"
                        )
            
            if not chunk_uploaded:
                raise RuntimeError(f"Chunk-Upload bei Offset {upload_offset} fehlgeschlagen")
            
            upload_offset += len(chunk_data)
            
            # Inter-Chunk-Delay (gegen Rate-Limiting / Connection-Resets)
            chunk_delay = float(os.environ.get("PCLOUD_CHUNK_DELAY", "0.15"))  # 150ms default
            if chunk_delay > 0:
                import time
                time.sleep(chunk_delay)
            
            # Optional: Progress-Log (nur bei VERBOSE)
            if os.environ.get("PCLOUD_VERBOSE") == "1":
                pct = (upload_offset / file_size * 100) if file_size else 0
                print(f"[chunk] {fname}: {upload_offset}/{file_size} ({pct:.1f}%)", flush=True)
    
    # Schritt 3: Upload finalisieren
    # WICHTIG: folderid + name (nicht path) - laut pCloud API Empfehlung
    save_params = {
        "access_token": cfg["token"],
        "uploadid": uploadid,
    ...
    r = session.post(f"{base_url}/upload_save", params=save_params, timeout=(60, 60))
    try:
        j = r.json()
    except Exception:
        raise RuntimeError(f"upload_save: HTTP {r.status_code}, kein JSON")
    
    if j.get("result") != 0:
        raise RuntimeError(f"upload_save failed: {j}")
...
def effective_config(env_file: Optional[str]=None,
    ...
    # Standard .env
    if default_env:
        # Unterstütze beide: PCLOUD_TIMEOUT und PCLOUD_TIMEOUT_SECS
        timeout_val = default_env.get("PCLOUD_TIMEOUT") or default_env.get("PCLOUD_TIMEOUT_SECS")
        cfg.update({
            "host": default_env.get("PCLOUD_HOST", cfg["host"]),
            "port": int(default_env.get("PCLOUD_PORT", cfg["port"])),
            "timeout": int(timeout_val or cfg["timeout"]),
    ...
    if prof_env:
        # Unterstütze beide: PCLOUD_TIMEOUT und PCLOUD_TIMEOUT_SECS
        timeout_val = prof_env.get("PCLOUD_TIMEOUT") or prof_env.get("PCLOUD_TIMEOUT_SECS")
        cfg.update({
    ...
    # ENV (Unterstütze beide: PCLOUD_TIMEOUT und PCLOUD_TIMEOUT_SECS)
    timeout_val = os.environ.get("PCLOUD_TIMEOUT") or os.environ.get("PCLOUD_TIMEOUT_SECS")
    cfg.update({
...
def _rest_get(cfg: dict, endpoint: str, params: dict | None = None):
    ...
    SECURITY: Fängt HTTP-Errors ab und entfernt access_token aus URL!
    """
    import requests
    import re
    base = _rest_base(cfg)
    ...
    try:
        ...
        msg = str(e)
        msg = re.sub(r'access_token=[^&\s]+', 'access_token=***HIDDEN***', msg)
        raise RuntimeError(f"REST {endpoint} HTTP Error: {msg}") from None
    except requests.exceptions.RequestException as e:
        # Andere Request-Errors (Timeout, Connection, etc.)
        msg = str(e)
        msg = re.sub(r'access_token=[^&\s]+', 'access_token=***HIDDEN***', msg)
...
def _rest_post(cfg: dict, endpoint: str, data: dict | None = None, files: dict | None = None):
    ...
    try:
        ...
        msg = str(e)
        msg = re.sub(r'access_token=[^&\s]+', 'access_token=***HIDDEN***', msg)
        raise RuntimeError(f"REST {endpoint} Error: {msg}") from None
...
def get_textfile(cfg: dict, *, path: str | None = None, fileid: int | None = None,
    ...
    Nutzt Keep-Alive Session für bessere Stabilität.
    """
    import json as _json
...
def upload_text_as_file(cfg: Dict[str, Any], text: str, dest_path: str, filename: str | None = None) -> Dict[str, Any]:
    ...
    ensure_path_cached(cfg, parent_dir)
    md_parent = stat_folder_cached(cfg, path=parent_dir)
...
def call_with_backoff(func, *args, attempts: int = 5, max_sleep: float = 60.0, **kwargs):
    ...
    Wenn ein cfg-dict mit 'binary_max_attempts' in args ist,
    wird attempts auf diesen Wert begrenzt (ermöglicht fast-fail für nicht-kritische Pfade).
    """
    import time
    # Suche in allen args nach einem cfg-dict mit binary_max_attempts (robust gegen API-Änderungen)
    for arg in args:
        if isinstance(arg, dict) and "binary_max_attempts" in arg:
            attempts = min(attempts, arg.get("binary_max_attempts", attempts))
            break
    last_exc = None
...
def userinfo(cfg: Dict[str, Any]) -> Dict[str, Any]:
    """
    Ruft /userinfo (REST) auf und gibt das JSON zurück.
    Nutzt den Host aus der Config (eapi.pcloud.com oder api.pcloud.com).
    Erwartet {"result":0, ...} bei Erfolg.
    """
    base = _rest_base(cfg)  # Nutzt cfg["host"] statt hardcoded!
    params = {
    ...
    session = _get_session()
    r = session.get(f"{base}/userinfo", params=params, timeout=int(cfg.get("timeout", 30)))
    try:
        j = r.json()
    except Exception:
...
# === FolderID Persistent Cache =============================================
# aktiviert über ENV (Default-Pfade/TTL sind sinnvoll)
_FIDCACHE_PATH = os.environ.get("PCLOUD_FOLDERID_CACHE",
                                "/opt/apps/pcloud-tools/main/state/folderid-cache.json")
_FIDCACHE_TTL  = int(os.environ.get("PCLOUD_FIDCACHE_TTL", "3600"))  # Sekunden

def _fidcache_load() -> dict:
    try:
        with open(_FIDCACHE_PATH, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {}

def _fidcache_save(cache: dict) -> None:
    try:
        os.makedirs(os.path.dirname(_FIDCACHE_PATH), exist_ok=True)
        with open(_FIDCACHE_PATH, "w", encoding="utf-8") as f:
            json.dump(cache, f, ensure_ascii=False)
    except Exception:
        pass

def _fidcache_get(path: str) -> int | None:
    path = _norm_remote_path(path)
    cache = _fidcache_load()
    ent = cache.get(path)
    if not ent: return None
    ts = int(ent.get("ts") or 0)
    if (time.time() - ts) > _FIDCACHE_TTL:
        return None
    return int(ent.get("folderid") or 0) or None

def _fidcache_put(path: str, folderid: int) -> None:
    path = _norm_remote_path(path)
    cache = _fidcache_load()
    cache[path] = {"folderid": int(folderid), "ts": int(time.time())}
    _fidcache_save(cache)

def stat_folder_cached(cfg: dict, *, path: str) -> dict:
    """
    Holt Folder-Metadaten, nutzt persistenten Cache für folderid.
    Fällt bei Miss/Expired auf reguläres stat_folder zurück.
    """
    path = _norm_remote_path(path)
    fid = _fidcache_get(path)
    if fid is not None and fid >= 0:
        try:
            md = stat_folder(cfg, folderid=int(fid))
            if md and (md.get("path") == path or md.get("name")):
                return md
        except Exception:
            pass  # Fallback auf stat_folder by path
    md = stat_folder(cfg, path=path)
    if md:
        _fidcache_put(path, int(md.get("folderid") or 0))
    return md

def ensure_path_cached(cfg: dict, path: str) -> int:
    """
    ensure_path mit persistentem Cache; liefert folderid des Zielordners.
    """
    path = _norm_remote_path(path)
    # 1) Cache-Hit?
    fid = _fidcache_get(path)
    if fid is not None:
        return fid
    # 2) Elternkette iterativ sicherstellen (nutzt bestehende ensure_path)
    ensure_path(cfg, path)
    md = stat_folder(cfg, path=path)
    folderid = int(md.get("folderid") or 0)
    if folderid:
